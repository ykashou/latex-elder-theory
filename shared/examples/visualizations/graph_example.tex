...
\addlegendentry{$\elderstructure{1}(x) = x^2$}

\addplot[color=DarkGray, thick, domain=0:5, samples=100]{exp(x/2)};
\addlegendentry{$\elderstructure{2}(x) = e^{x/2}$}

\addplot[color=LemmaGreen!80, thick, domain=0:5, samples=100]{x^2*sin(deg(x))};
\addlegendentry{$\elderstructure{3}(x) = x^2\sin(x)$}

\end{axis}
\end{tikzpicture}
\caption{Behavior of the first three Elder structural functions}
\end{figure}
...
\begin{axis}[
    ...
    view={30}{30},
    colormap={elder}{color(0)=(DarkSkyBlue) color(1)=(LemmaGreen)}
]
...
\begin{axis}[
    width=10cm,
    height=8cm,
    xlabel={$\elderstructure{1}$},
    ylabel={$\elderstructure{2}$},
    grid=major,
    xmin=-2, xmax=2,
    ymin=-2, ymax=2
]
...
\begin{axis}[
    ...
    height=8cm,
    xlabel={Parameter $\mu$},
    ylabel={Equilibrium $\elderstructure{*}$},
    grid=major,
    xmin=0, xmax=4,
    ymin=-2, ymax=2
]
...
\end{axis}
\end{tikzpicture}
\caption{Bifurcation diagram for the Elder equation $\dot{\elderstructure{}} = \mu \elderstructure{} - \elderstructure{}^3$}
\end{figure}
...
\begin{tikzpicture}[
    scale=1.2,
    structure node/.style={circle, draw=DarkSkyBlue, fill=DarkSkyBlue!20, thick, minimum size=0.8cm},
...
% Nodes
\node[structure node] (A1) at (0,0) {$A_1$};
\node[structure node] (A2) at (2,1) {$A_2$};
\node[structure node] (A3) at (4,0) {$A_3$};
\node[structure node] (A4) at (6,1) {$A_4$};
\node[structure node] (A5) at (8,0) {$A_5$};
...
\end{tikzpicture}
\caption{Network representation of Structure-Elder interactions}
\end{figure}
