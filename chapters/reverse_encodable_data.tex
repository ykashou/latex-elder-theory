...
\begin{tikzpicture}
  ...
  % Frame indicators (reference frames stay constant)
  \draw[<->, dashed] ($(original.north) + (0,0.8)$) -- node[above] {Elder Reference Frame $\mathcal{F}_E$ (fixed)} ($(level3.north) + (0,0.8)$);
  \draw[<->, dashed] ($(original.south) + (0,-0.8)$) -- node[below] {Mentor Reference Frame $\mathcal{F}_M$ (fixed)} ($(level3.south) + (0,-0.8)$);
