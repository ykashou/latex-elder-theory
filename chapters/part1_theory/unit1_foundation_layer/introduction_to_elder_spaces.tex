...
\textbf{Part 2: Non-Commutative Algebra Structure}
We now establish that the multiplication operation $\star$ makes $\elder{d}$ a non-commutative algebra:
...
\begin{enumerate}
    ...
    \item \textbf{Non-Commutativity}: In general, $x \star y \neq y \star x$. This is essential for the hierarchical structure since it allows for directional influence between entities at different levels of the hierarchy.
\end{enumerate}
...
For any $x \in \elder{d}$ with basis expansion $x = \sum_{i=1}^{d} c_i \elderstructure{i}$ where $c_i = \lambda_i e^{i\theta_i}$, define:
\begin{equation}
\mathrm{tr}_E(x) = \sum_{i=1}^{d} \lambda_i e^{i\theta_i}
\end{equation}
