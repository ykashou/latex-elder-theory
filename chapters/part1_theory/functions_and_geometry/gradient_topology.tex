...
\subsection{Mathematical Model of Group Formation}
...
\begin{equation}
\frac{d\phi_i}{dt} = \omega_i + \sum_j K_{ij} \sin(\phi_j - \phi_i \cdot \mu_{ij})
\end{equation}

where $\omega_i$ is the intrinsic frequency of parameter $i$, and $K_{ij}$ is the coupling strength between parameters.
...
\begin{proposition}[Dimensionality-Coherence Scaling Law]
For a system with $|\Theta|$ parameters, the effective dimensionality scales with phase coherence according to:
\begin{equation}
d_{\text{eff}}(\Phi) \approx |\Theta|^{1-\Phi} \cdot (\log|\Theta|)^{\Phi}
\end{equation}
for $\Phi \in [0,1]$.
\end{proposition}
