...
\begin{tikzpicture}[scale=0.7]
    % Base manifold - simple ellipse
    \draw[thick] (0,0) ellipse (3 and 0.8);
    \node at (0,-1.5) {Base manifold $M$ (conceptual space)};
    
    % Total space - simple line at top
    \draw[thick] (-3.5,3.5) -- (3.5,3.5);
    \node at (0,4) {Total space $E$ (parameter space)};
    
    % Simple vertical fibers
    \draw[thick] (-2.5,0.5) -- (-2.5,3);
    \draw[thick] (-1,0.5) -- (-1,3);
    \draw[thick] (0.5,0.5) -- (0.5,3);
    \draw[thick] (2,0.5) -- (2,3);
    
    % Projection arrow
    \draw[->, thick] (1,2.5) -- (1,1.2);
    \node at (1.5,1.8) {$\pi$};
    
    % Fiber label
    \node at (-3.5,2) {Fibers (phase space)};
