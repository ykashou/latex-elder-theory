...
\begin{tikzpicture}[scale=1.1]
    ...
    \node[font=\small, align=left] at (-4.5,-3.2) {
        \textcolor{blue!70}{● Elder resonance}\\
        \textcolor{red!70}{● Mentor resonance}\\
        \textcolor{green!70}{● Erudite resonance}
    };
...
\begin{tikzpicture}[scale=1.0]
    % Enhanced coordinate system with proper scaling
    \draw[->, very thick] (0,0) -- (11,0) node[right, font=\large] {Phase Coherence $\Phi$};
    \draw[->, very thick] (0,0) -- (0,8.5) node[above, font=\large] {Effective Dimensionality $d_{\text{eff}}$};
    
    % Sophisticated background gradient showing dimensional landscape
    \foreach \y in {0.1,0.2,...,8.0} {
        \fill[gray!5, opacity=0.3] (0,\y) rectangle (10.5,\y+0.1);
    }
    
    % Enhanced axis markings
    \foreach \x in {0,2,4,6,8,10} {
        \draw[thick] (\x,0.15) -- (\x,-0.15) node[below, font=\small] {$\x/10$};
        \draw[gray!30, thin] (\x,0) -- (\x,8);
    }
    
    % Logarithmic y-axis with mathematical precision
    \foreach \y/\label in {1/$\log|\Theta|$, 2.5/$\sqrt{|\Theta|}$, 4/$|\Theta|^{2/3}$, 6/$|\Theta|^{4/5}$, 7.5/$|\Theta|$} {
        \draw[thick] (-0.15,\y) -- (0.15,\y) node[left, font=\small] {\label};
        \draw[gray!30, thin] (0,\y) -- (10.5,\y);
    }
    
    % Main dimensionality reduction curve with mathematical sophistication
    \draw[ultra thick, blue!80, domain=0:10.5, smooth, variable=\x] 
        plot ({\x}, {7.5 - 6.5*exp(-3*(10.5-\x)/10.5) + 0.8*exp(-8*\x/10.5)});
    
    % Secondary curves showing uncertainty bounds
    \draw[thick, blue!40, dashed, domain=0:10.5, smooth, variable=\x] 
        plot ({\x}, {7.8 - 6.8*exp(-3*(10.5-\x)/10.5) + 0.9*exp(-8*\x/10.5)});
    \draw[thick, blue!40, dashed, domain=0:10.5, smooth, variable=\x] 
        plot ({\x}, {7.2 - 6.2*exp(-3*(10.5-\x)/10.5) + 0.7*exp(-8*\x/10.5)});
    
    % Critical phase transition regions with mathematical boundaries
    \fill[red!10, opacity=0.4] (0,0) rectangle (3,8.5);
    \fill[yellow!15, opacity=0.4] (3,0) rectangle (7,8.5);
    \fill[green!10, opacity=0.4] (7,0) rectangle (10.5,8.5);
    
    % Phase transition markers with mathematical precision
    \draw[red!70, very thick, dashed] (3,0) -- (3,8.5);
    \draw[green!70, very thick, dashed] (7,0) -- (7,8.5);
    \node[red!70, font=\small] at (3,-0.4) {$\Phi_1 = 0.3$};
    \node[green!70, font=\small] at (7,-0.4) {$\Phi_c = 0.7$};
    
    % Mathematical regime annotations with enhanced styling
    \node[align=center, font=\footnotesize\bfseries, text width=2.5cm] at (1.5,7) {
        \textcolor{red!80}{Traditional\\Gradient Regime}\\
        \vspace{0.1cm}
        $d_{\text{eff}} \approx |\Theta|$\\
        Minimal coherence
    };
    
    \node[align=center, font=\footnotesize\bfseries, text width=2.5cm] at (5,5.5) {
        \textcolor{orange!80}{Transition\\Regime}\\
        \vspace{0.1cm}
        $d_{\text{eff}} \sim |\Theta|^{\alpha}$\\
        $0.5 < \alpha < 1$
    };
    
    \node[align=center, font=\footnotesize\bfseries, text width=2.5cm] at (8.5,2.5) {
        \textcolor{green!70}{Resonant\\Regime}\\
        \vspace{0.1cm}
        $d_{\text{eff}} \approx \log|\Theta|$\\
        High coherence
    };
    
    % Sophisticated arrows showing dimensional collapse
    \draw[->, ultra thick, red!60] (1.5,6.2) to[out=-30,in=180] (3.5,4.8);
    \draw[->, ultra thick, orange!70] (5,4.8) to[out=-30,in=180] (7.5,2.8);
    \draw[->, ultra thick, green!60] (8.5,1.8) to[out=0,in=135] (9.8,1.2);
    
    % Mathematical function label with enhanced typography
    \node[blue!80, font=\large\bfseries] at (9.5,4) {$d_{\text{eff}}(\Phi)$};
    
    % Theoretical bounds with mathematical rigor
    \draw[gray!60, thick, dotted] (0,1) -- (10.5,1);
    \draw[gray!60, thick, dotted] (0,7.5) -- (10.5,7.5);
    \node[gray!60, font=\tiny] at (10.8,1) {Lower bound};
    \node[gray!60, font=\tiny] at (10.8,7.5) {Upper bound};
    
    % Enhanced mathematical annotations
    \node[font=\tiny, align=left] at (0.5,0.3) {
        Spectral gap analysis:\\
        $\lambda_{\text{gap}} \propto \Phi^2$
    };
    
    % Parameter space visualization indicators
    \foreach \x in {1,3,5,7,9} {
        \foreach \y in {0.5,1.5,2.5} {
            \fill[blue!20, opacity=0.6] (\x,{7.5 - 6.5*exp(-3*(10.5-\x)/10.5) + 0.8*exp(-8*\x/10.5) + \y*0.1}) circle (0.02);
        }
    }
    
\end{tikzpicture}
\caption{Advanced visualization of effective parameter dimensionality as a function of phase coherence in the Elder Heliosystem. The sophisticated curve demonstrates the superlinear dimensional collapse from full parameter space $|\Theta|$ to logarithmic scaling $\log|\Theta|$ as coherence increases. Colored regions indicate distinct mathematical regimes with their characteristic scaling laws, while theoretical bounds and spectral gap analysis provide rigorous mathematical foundation.}
