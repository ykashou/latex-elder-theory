...
\begin{tikzpicture}[scale=1.0]
    ...
    \foreach \angle/\time in {0/t_0, 45/t_1, 90/t_2, 135/t_3, 180/t_4, 225/t_5}{
        \ifnum\angle=90
            \node[circle, fill=gray!40, minimum size=0.7cm] (erudite\angle) at ($(mentor90) + (\angle:1cm)$) {$\mathcal{E}r$};
            \draw[dotted, thick] (mentor90) -- (erudite\angle);
        \else
            \filldraw[gray!40] ($(mentor90) + (\angle:1cm)$) circle (0.07cm);
        \fi
    }
    ...
    \draw[<-] (0,-3.5) -- (0,-1.5) node[below] at (0,-3.5) {Hierarchical orbits preserve\\entity relationships};
\end{tikzpicture}
