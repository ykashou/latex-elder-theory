...
\begin{tikzpicture}[node distance=1.8cm, scale=0.8, transform shape]
    % Nodes
    \node[draw, rounded corners, fill=blue!10] (complex) {Complex-Valued Computation};
    \node[draw, rounded corners, fill=green!10, below left=of complex] (helio) {Heliomorphic Transform};
    \node[draw, rounded corners, fill=green!10, below right=of complex] (orbital) {Orbital Dynamics};
    \node[draw, rounded corners, fill=orange!10, below=of helio] (shell) {Shell Operations};
    \node[draw, rounded corners, fill=orange!10, below=of orbital] (gradient) {Gradient \& Optimization};
    \node[draw, rounded corners, fill=blue!10, right=of orbital] (field) {Knowledge Field};
    \node[draw, rounded corners, fill=green!10, right=of field] (spectral) {Spectral Analysis};
    \node[draw, rounded corners, fill=green!10, below=1cm of spectral] (geometry) {Differential Geometry};
    \node[draw, rounded corners, fill=orange!10, below=of shell] (loss) {Loss Functions};
    \node[draw, rounded corners, fill=orange!10, below=of geometry] (info) {Information Theory};
    \node[draw, rounded corners, fill=red!10, below right=of gradient] (transfer) {Cross-Domain Transfer};
    \node[draw, rounded corners, fill=red!10, below=of info] (hardware) {Hardware Optimization};
    
    % Connections
    \draw[->] (complex) -- (helio);
    \draw[->] (complex) -- (orbital);
    \draw[->] (complex) -- (field);
    \draw[->] (helio) -- (shell);
    \draw[->] (orbital) -- (gradient);
    \draw[->] (shell) -- (loss);
    \draw[->] (field) -- (spectral);
    \draw[->] (field) -- (geometry);
    \draw[->] (spectral) -- (info);
    \draw[->] (geometry) -- (gradient);
    \draw[->] (loss) -- (transfer);
    \draw[->] (orbital) -- (transfer);
    \draw[->] (info) -- (transfer);
    \draw[->] (gradient) -- (hardware);
    \draw[->] (shell) -- (hardware);
    \draw[->] (complex) -- (hardware);
    
    % Group high-level algorithms
    \draw[dashed, rounded corners, thick] (-3,-5.5) rectangle (3,-8.5);
    \node at (0,-5.2) {High-Level Training Algorithms};
    
    % Group mid-level operators
    \draw[dashed, rounded corners, thick] (-4,-0.2) rectangle (7,-5);
    \node at (1.5,0.1) {Mid-Level Mathematical Operators};
    
    % Group low-level primitives
    \draw[dashed, rounded corners, thick] (-4.5,0.3) rectangle (7.5,1.8);
    \node at (1.5,2.1) {Low-Level Computational Primitives};
\end{tikzpicture}
